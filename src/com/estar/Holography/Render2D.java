...
public class Render2D {
    ...
    private final float[] mVertices2DDataOrign = {
            // X, Y, Z, U, V
    		-1.0f, -1.0f, 0,  0.f, 1.f,
    ...
    int vCount=6;   
    public Render2D(SurfaceView mv, float wper, float hper) {
    ...
    public void setPercent(float wper, float hper) {
    	for( int i = 0; i < 6; i++ ) {
    		mVerticesData[5 * i + 0] = mVertices2DDataOrign[5 * i + 0] * wper;
    		mVerticesData[5 * i + 1] = mVertices2DDataOrign[5 * i + 1] * hper;
    		
    		if( mVertices2DDataOrign[5 * i + 3] == 0 ) {
    			mVerticesData[5 * i + 3] = (1.0f - wper) / 2.0f;
    		}
    		else {
    			mVerticesData[5 * i + 3] = 1.0f - (1.0f - wper) / 2.0f;
    		}
    		if( mVertices2DDataOrign[5 * i + 4] == 0 ) {
    			mVerticesData[5 * i + 4] = (1.0f - hper) / 2.0f;
    		}
    		else {
    			mVerticesData[5 * i + 4] = 1.0f - (1.0f - hper) / 2.0f;
    		}
    	}
    }
    ...
    {        
    	if( mProgram <= 0 ) {
    		Utils.showLogDebug("no program to draw");
    		return;
    	}
 ...
 //      GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4);
    ...
    private void checkGlError(String op) {
        ...
        while ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
            Utils.showLogDebug(op + ": glError " + error);
        }
    }
